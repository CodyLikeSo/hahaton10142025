...
async def upload_categories(db: AsyncSession = Depends(async_get_db)):
    ...
    if not file_path.exists():
        raise HTTPException(status_code=404, detail=f"Файл {EXCEL_FILE_NAME} не найден в директории проекта")
    ...
    try:
        ...
        if not expected_excel_columns.issubset(df.columns):
            ...
            raise HTTPException(status_code=400, detail=f"В файле отсутствуют столбцы: {missing}")
        ...
        return {"message": f"Успешно загружено {len(categories)} записей из файла {EXCEL_FILE_NAME}"}
